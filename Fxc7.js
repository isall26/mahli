...
prefix = "!"
name = "~ BOT-YOE"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN BOT-YOE😁"
rmenu = "HAI TEMEN BOT-YOE👋 JANGAN LUPA DONASI YAA:)"
botinfo = "UNTUK INVITE BOT SILAHKAN DONASI DULU YAA:)"
limitt = 15
memberLimit = 2
ban = []
premium = ["6281229247198@s.whatsapp.net"]
		...
		case 'daftar':
			...
			if (!isPublic) return reply(mess.only.publikG)
                ...
                const tanggal = `${thisDay}, ${day} - ${myMonths[bulan]} - ${year}`
					await costum(help(prefix, instagram, yt, name, pushname2, user, limitt, uptime, jam, tanggal), text, mahliGans, rmenu)
    				break 
    			case 'infobot':
    				await costum(bottt(prefix), text, mahliGans, botinfo)
    				break
    			case 'profile':
    				frhan.updatePresence(from, Presence.composing)
    				if (!isPublic) return reply(mess.only.publikG)
